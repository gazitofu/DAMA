...
- 자동 적용 정책은 `context-correction-2`다. 모든 소유 Turn ID가 순서대로 정확히 한 번 반환되어야 하며, certain=true에 더해 코드의 제한된 형식 동등성이 확인될 때만 반영한다. 비교는 유효한3자리 천단위 comma 양옆 ASCII 공백, 독립된 안되니까/안되는데/안해도의 안 뒤 공백, 숫자 뒤 조/억/만/천/백과 원 사이 공백에만 한정한다. 숫자 계산·단위 변환·부호/구두점 삭제·전체 공백 제거는 하지 않는다. 예: `1 ,000억→1,000억` 허용, `3 ,40억→3,40억` 보류. 이는 형식 정책이며 발화의 정답 검증은 아니다.
- 새 어휘/약어/구두점 또는 다른 내부공백 변경은 원문 유지+후보/이유+노란 이벤트로 남긴다. LLM certain/reason·참고 자료에 후보가 존재한다는 사실만으로 허용하지 않는다. 자동 어휘 교정 확대는 앱이 검증하는 수정별 근거 계약과 평가가 필요한 후속이다. v1의 보호 토큰 동등 비교는 제거했다. 새 정책은 새 응답 검증에 적용하고 기존 저장 교정본의 적용 판정을 재해석하거나 사용자 수정본을 덮지 않는다.
