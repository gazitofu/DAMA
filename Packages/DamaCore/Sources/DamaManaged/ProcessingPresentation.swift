...
public struct ProcessingPresentation: Sendable {
    ...
    public init(run: ManagedRun?, tracking: Bool, preparing: Bool = false, scriptSaved: Bool = false) {
        ...
        let activeStages = ["queued", "uploading", "submitting", "remotePending", "normalizing"]
        ...
        case "remotePending":
            title = run.remoteStatus == "running" ? "전사와 화자 분리가 진행 중입니다" : "서버에서 처리 순서를 기다리고 있습니다"
        ...
        case "waitingForBilling":
            title = "결제 상태를 확인해 주세요"; detail = "pyannote.ai 계정 확인 후 변환을 재개하세요."; step = nil
